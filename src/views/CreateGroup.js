...
class CreateGroup extends Component {
  ...
  async  componentDidMount() {
    ...
    createGroups = async () => {
        ...
        const {Groupname,selectedValue}= this.state;
        const image = {
            uri: photoURI,
            type: 'image/jpeg',
          ...
          if(this.state.selectedValue.length <=1){
            this.state.selectedValue.forEach((item) => {
            formdata.append('groupMember[0]',item.id);
          })
          }
          else{
            this.state.selectedValue.forEach((item) => {
                formdata.append('groupMember', item.id)
             })
          }
        //   formdata.append('groupMember[]',selectedValue.map(e =>e.id));
        
        if (this.state.selectedValue.length > 0 && !_.isEmpty(this.state.Groupname)) {
            return  fetch(global.creategroup, {
                method: 'POST',
                headers: {
    ...
    render() {
        ...
        // console.log('final data to send ', this.state.selectedValue[0].id);
        return (
            <View style={{ flex: 1 }}>
