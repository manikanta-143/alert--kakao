...
export default class GroupDetails extends Component {
  ...
  _getGroupDetails = async () => {
    ...
    await fetch(`https://kakaonodeapp.herokuapp.com/mission21/groups/${id}`, {
      ...
      }).catch(error => alert('error in get request groupid', error))
  };
  _groupImage = async () =>{
    var id = await this.props.navigation.state.params.Gid;
    var token = await AsyncStorage.getItem('STORAGE_KEY');
    ...
    await fetch(`https://kakaonodeapp.herokuapp.com/mission21/groupImage/${id}`, {
      ...
      }).catch(error => console.log('error in get request group image', error))
  }
//  componentDidUpdate(){
//    this._groupImage();
//  }
  ...
  checkBoxChanged(id, value, user) {
    ...
    if (value) {
      ...
      //  arr.push(...this.state.selectedValue,user);
      //  var obj =_.uniqBy(arr, 'phoneMemberId');

      //  _.uniqWith(arr, _.isEqual);
      //  _.uniqBy(arr, 'phoneMemberId');
      //  var obj =_.uniqBy(arr, 'phoneMemberId');
  ...
  mcheckBoxChanged(id, value, user) {
    const { checkBoxChecked, selectedValue } = this.state;
    this.setState({ visible: false})
      ...
      >
        ...
        >
          ...
          <CheckBox
            ...
            checked={this.state.checkBoxChecked[user.phoneMemberId]}
            // && this.state.value}
  ...
  RemoveAll =() =>{
    this.setState( { visible : false} , ()=> {
      if(!this.state.visible){
          return _.map(this.state.groupData, (user, index) => {
              return  this.setState({ checkBoxChecked : [false],selectedValue:[] })
            })
          }
    })
  }
  selectAll = () =>{
   this.state.groupData.map((user,index) => {
      return this.mcheckBoxChanged(user.phoneMemberId, this.state.checkBoxChecked[true],user) &&  this.state.checkBoxChecked[true]
     })
 ...
 DeleteUser = async() =>{
  ...
  await fetch(`https://kakaonodeapp.herokuapp.com/mission21/groups/remove`, {
    ...
    .then(() => alert('User removed from group'),
    this._getGroupDetails()
    )
    .catch((err) => alert(err))
    .done();
 ...
 updateGroupName = async() =>{
  ...
  FullImage = () =>{
    return this.props.navigation.navigate('GroupIcon',{
      groupImage: this.state.groupImage,
      Gid: this.state.GroupId
    })}
  
  render() {
    console.log(this.state.selectedValue,'final list')
    console.log(this.state.selectedValue.length,'final list length');
          ...
          <SafeAreaView style={{ flex: 1, backgroundColor: 'rgba(241,240,241,1)' }} >
            <ImageBackground source={Bg_Img} style={styles.backgroundContainer}>
              ...
              >
                ...
                >
                  <TouchableOpacity medium primary style={styles.imageThumbnail} 
                  // onPress={this.onSpeakerPhotoPress.bind(this)}
                  // onPress={this.props.navigation.navigate('GroupIcon',{
                  //   groupImage: this.state.groupImage
                  // })}
                  onPress={this.FullImage}
                  >
                    <Thumbnail circle medium source={{
                      uri: this.state.image == '' ? this.state.groupImage : this.state.imageURI
              ...
              <View style={{ flex: 1, alignItems: 'flex-end', justifyContent: 'center' }}>
                  ...
                  >
                    <MenuItem onPress={()=> Alert.alert('Edit phonebook user')} >Edit</MenuItem>
                    <MenuDivider />
                    <View style={{flex: 1,flexDirection: 'row',alignItems: 'center',justifyContent: 'space-between'}}>
                    <MenuItem onPress={this.DeleteUser} title={this.state.name}>Remove</MenuItem>
              ...
              <View style={{ flexDirection: 'row', paddingBottom: 10,backgroundColor: 'transparent',justifyContent: 'center',alignItems: 'center'}}>
              ...
              <Icon
                    ...
                    size={30}
                    type="materialicons"
                    iconStyle={{  color: "#FF4500" }}
                    onPress={this.RemoveAll} 
                  />
               <Text style={{fontSize : 8}}>Deselect all</Text>      
              </View>
              <View style={{ flexDirection: 'column',padding: 20,justifyContent: 'center',alignItems: 'center'}}>
             ...
             <Icon
                  ...
                  //  iconStyle={{  color: "#FF4500" }}
                   ...
                   iconStyle={ this.state.visible  ? {  color: '#ebebeb'} :{  color: '#FF4500'} }
                 />
                 <Text style={{fontSize : 8}}>Select all</Text>   
